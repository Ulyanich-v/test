...
class DomParser {
    parseSA1Form() {
        const $form = $('#mrForm');
        const $bannerText = $('.mrBannerText');
        const $questionTable = $('.mrQuestionTable');
        const $questionText = $('.mrQuestionText');
        const $radioButtonLabel = $('.mrSingleText');
        const $progress = $('.mrProgressText');
        const jsonParseData = {
            hiddenInputs: [],
            radioInputs: [],
            questionText: '',
        ...
        jsonParseData.form.action = $form.attr('action');
        jsonParseData.form.method = $form.attr('method');
        jsonParseData.bannerText = $bannerText.children().text();
        jsonParseData.questionText = $questionText.text();
        $form.children().each((index, element) => {
            if (element.tagName === 'INPUT') {
                jsonParseData.hiddenInputs.push({
                    name: $(element).attr('name'),
                    value: $(element).val(),
                });
            }
        });
        jsonParseData.progress = $progress.text();
        $questionTable.children().each((index, element) => {
            const $radioElm = $(element).find('.mrSingle')[0];
            const $radioTitle = $(element).find('.mrSingleText');
            jsonParseData.radioInputs.push({
...
class ReactApp {
    constructor() {
        ...
        this.rootComponent = (React.createElement(react_redux_1.Provider, { store: store },
            React.createElement(react_router_1.Router, { history: history, routes: routes })));
    }
    render() {
        ReactDOM.render(this.rootComponent, document.getElementsByTagName('body')[0]);
    }
...
const DomParser_1 = __webpack_require__("./DomParser.ts");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const htmlParseData = parser.parseSA1Form();
...
function getRoutes() {
    return (React.createElement(react_router_1.Route, { path: "/", component: App_1.App }));
}
Object.defineProperty(exports, "__esModule", { value: true });
